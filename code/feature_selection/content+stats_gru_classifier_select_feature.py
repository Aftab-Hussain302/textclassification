...
if __name__ == '__main__':
    ...
    for flaw_index in range(4, 5):
        ...
        # 在每个flaw下分别计算指标
        for feature_index in range(16, 35):
            ...
            with open('experiment_log.csv', 'a', newline='', encoding='utf-8-sig') as t_file:
                ...
                csv_writer.writerow((str(flaw_index), 'feature' + str(feature_index), np.mean(kfold_precision), np.mean(kfold_recall), np.mean(kfold_f1_score), np.mean(kfold_acc), np.mean(kfold_TNR), np.mean(kfold_training_acc, axis=0), np.mean(kfold_training_loss, axis=0)))
