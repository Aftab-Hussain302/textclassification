...
if __name__ == '__main__':
    ...
    for flaw_index in flaw_sequence:
        ...
        plt.legend(loc='upper right')  # acc: lower right  loss:upper right
        plt.locator_params('x',nbins=len(text_statistic))  # 刻度个数
        plt.xlim([-1,len(text_statistic)])
        plt.ylim([-0.05, 0.55])  # acc: 0.7, 1.03  loss: -0.05, 0.55
        ...
        img_index += 1
    # plt.show()
    plt.savefig('/results/feature_set_selection_loss.png')
